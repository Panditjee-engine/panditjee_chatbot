...
def analyze_combinations(natal_chart):
    detected_combinations = []
    if "Jupiter" in natal_chart and "Venus" in natal_chart:
        jupiter_pos = natal_chart["Jupiter"]
        venus_pos = natal_chart["Venus"]
        if abs(jupiter_pos - venus_pos) % 360 <= 10:
            detected_combinations.append("Wealth Combination 2")

    if "Venus" in natal_chart and "7th Lord" in natal_chart:
        venus_pos = natal_chart["Venus"]
        lord_pos = natal_chart["7th Lord"]
        if venus_pos // 30 == 6:
            detected_combinations.append("Marriage Combination 1")

    return detected_combinations

# Nakshatra Analysis
def nakshatra_analysis(planet_positions):
    nakshatra_predictions = {}
    for planet, position in planet_positions.items():
        nakshatra_index = int(position // 13.3333) % 27
        nakshatra_name = list(NAKSHATRA_DETAILS.keys())[nakshatra_index]
        nakshatra_info = NAKSHATRA_DETAILS.get(nakshatra_name, {})
        nakshatra_predictions[planet] = {
            "Nakshatra": nakshatra_name,
            "Ruler": nakshatra_info.get("Ruler"),
            "Symbol": nakshatra_info.get("Symbol"),
            "Deity": nakshatra_info.get("Deity"),
            "Characteristics": nakshatra_info.get("Characteristics")
        }
    return nakshatra_predictions
...
# Main function to demonstrate integrations
def main():
    ...
    print("\nNakshatra Analysis:")
    nakshatra_data = nakshatra_analysis(natal_chart)
    for planet, details in nakshatra_data.items():
        print(f"{planet}: Nakshatra - {details['Nakshatra']}, Ruler - {details['Ruler']}, Characteristics - {details['Characteristics']}")

    print("\nDetected Combinations:")
    combinations = analyze_combinations(natal_chart)
